...
void HandleInputChooseMove(u32 battler)
{
    ...
    {
        if (gBattleStruct->gimmick.usableGimmick[battler] != GIMMICK_NONE && !HasTrainerUsedGimmick(battler, gBattleStruct->gimmick.usableGimmick[battler]) && !(gBattleStruct->gimmick.usableGimmick[battler] == GIMMICK_Z_MOVE && GetUsableZMove(battler, moveInfo->moves[gMoveSelectionCursor[battler]]) == MOVE_NONE))
        {
...
{
    // Instead of giving EXP to the Pokémon, add the EXP as money to the player's funds.
    // The EXP value is typically passed in gBattleResources->bufferA[battler][2..5] as a 32-bit int.
    u32 exp = 0;
    exp = gBattleResources->bufferA[battler][2];
    exp |= gBattleResources->bufferA[battler][3] << 8;
    exp |= gBattleResources->bufferA[battler][4] << 16;
    exp |= gBattleResources->bufferA[battler][5] << 24;

    // Add EXP as money to the player's funds
    AddMoney(&gSaveBlock1Ptr->money, exp);

    // Complete the controller action for this battler
